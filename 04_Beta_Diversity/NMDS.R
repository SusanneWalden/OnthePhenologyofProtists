...
OTU_Table = as.data.frame(read.csv2("AllSeason_Cerc_En_Final_OTU_R_7633_edit_plottogether_extended.csv",stringsAsFactors=TRUE,header = T))
...
rownames(data.scores_sep) = SampleMetadata$ï..SampleID
...
g = ggplot() + 
  ...
  #scale_fill_manual(values = c("#006d2c","#fd8d3c","#fed976","#d4b9da","#045a8d","#0570b0","#a6bddb","#8e8878","#524640"), 
                   # limits = c("Fresh Leaves","Bark","Arboreal Soil", "Deadwood","Hypnum","Orthotrichum","Lichen","Leaf Litter", "Soil")) +  
  #scale_fill_manual(values = c("#3cb44b","#f58231","#ffe119","#ffd8b1","#469990","#4363d8","#dcbeff","#9A6324","#800000"), 
                    #limits = c("Fresh Leaves","Bark","Arboreal Soil", "Deadwood","Hypnum","Orthotrichum","Lichen","Leaf Litter", "Soil")) + 
  ...
  facet_grid(cols = vars(SamplingDescription))+# you can use cols instead of rows to switch the hole graphic
  facet_wrap(~SamplingDescription, ncol=2)+# to create 4x4 figure
  theme(strip.text.x = element_text(size=16,face="bold"))+# change size of Samplingdescription
